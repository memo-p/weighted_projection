...
void ProjWBF(datatype* y, datatype* w, datatype* x, const int length,
             const double a) {
  // union DtB r1;
  ...
  for (j = i = 0; j < length; ++j) {
    if (y[j] > w[j] * p) {
      perm[i] = j;
      const datatype wy = w[j] * y[j];
      const datatype ww = w[j] * w[j];
      sumWY += wy;
      Ws += ww;
      p = (sumWY - a) / Ws;
      if (p <= ((wy - a) / (ww))) {
  ...
  for (depth = 7; depth >= 0; depth--) {
    ...
    const datatype wy = y[p_s] * w[p_s];
    const datatype ww = w[p_s] * w[p_s];
    lsumWY = wy;
    lWs = ww;
    tau = (lsumWY + sumWY - a) / (Ws + lWs);
    const datatype i_v = y[p_s] / w[p_s];
    const size_t i_b = ((const unsigned char*)&i_v)[depth];
    ++tmp[i_b];
    s[i_b] += wy;
    wbs[i_b] += ww;
    ...
    for (i = illength - 2; i >= 0; --i) {
      const size_t pi = perm[i];
      if (y[pi] > w[pi] * tau) {
        const datatype wy = y[pi] * w[pi];
        const datatype ww = w[pi] * w[pi];
        lsumWY += wy;
        lWs += ww;
        tau = (lsumWY + sumWY - a) / (Ws + lWs);
        const datatype i_v = y[pi] / w[pi];
        const size_t i_b = ((const unsigned char*)&i_v)[depth];
        s[i_b] += wy;
        wbs[i_b] += ww;
        ++tmp[i_b];
